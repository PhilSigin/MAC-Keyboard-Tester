...
from .permissions import ensure_capture_permissions
...
class TapCapture(CaptureBackend):
    ...
    def start(self, *, prompt_permissions: bool = True) -> tuple[bool, str]:
        try:
            from Quartz import (
        ...
        ok_perm, perm_msg = ensure_capture_permissions(prompt=prompt_permissions)
        if not ok_perm:
        ...
        if not tap:
            # Tap create can fail even after a prompt; re-ask and surface guidance.
